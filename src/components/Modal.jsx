...
function Modal(props) {
    ...
    let [placeholder, setPlaceHolder] = useState('Please Remember!!\nThis ...\nThat ...');
        ...
        <div className='w-full inline-block align-bottom bg-gray-100 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full'>
          <div className='bg-gray-100'>
            <div className='sm:flex sm:items-start'>
              <textarea value = {data}
              required
                     onChange={e => setData(e.target.value)} name='item' cols='30' rows='10' className='w-full  px-4 pt-5 pb-4 sm:p-6 sm:pb-4 bg-gray-100' placeholder={placeholder}></textarea>
            </div>
          </div>
          <div className='bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse'>
            ...
            onClick={(e) => dispatch(addToKeep(data))}
              ...
              disabled={data.toString().length <= 0 || data.toString().length > 5000}
            >
                Save
            </button>
            <button
